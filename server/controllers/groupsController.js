...
const getGroup = async (req, res) => {
    ...
    const group = await Groups.findOne({_id: id}).populate('members').populate('captain');
...
const getUserGroups = async (req, res) => {
    ...
    const groups = await Groups.find({members: userId}).populate('members').populate('captain');
    res.status(200).json(groups);
}

// Create a new group
const createGroup = async (req, res) => {
    const {name, picture, sports, members, captain} = req.body;
    ...
    try {
        const group = await Groups.create({name, picture, sports, members, captain});
        res.status(200).json(group);
    } catch (error) {
...
const removeMember = async (req, res) => {
    ...
    const updated = await Groups.findOneAndUpdate({_id: id}, {$pull: {members: member}});
